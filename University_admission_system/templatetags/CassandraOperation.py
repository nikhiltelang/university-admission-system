...
class CassandraManagement:
    def __init__(self):
        ...
        try:
            self.cloud_config= {
                    'secure_connect_bundle': '/home/nick/Downloads/secure-connect-university-db.zip'
            }
    ...
    def select_student_info(self,id):
        try:
            ...
            personal_details = session.execute("select * from university.personal_details where id={}".format(id))
            religion_details = session.execute("select * from university.religion_details where id={}".format(id))
            caste_details = session.execute("select * from university.caste_details where id={}".format(id))
            income_details = session.execute("select * from university.income_details where id={}".format(id))
            domicile_details = session.execute("select * from university.domicile_details where id={}".format(id))
            permanent_address_details = session.execute("SELECT * FROM university.permanent_address_details where id={}".format(id))
            correspondence_address_details = session.execute("SELECT * FROM university.correspondence_address_details where id={}".format(id))
            past_qualification_details = session.execute("SELECT * FROM university.past_qualification_details where id={}".format(id))
            applying_details = session.execute("SELECT * FROM university.applying_details where id={}".format(id))
    ...
    def select_admission_summery(self):
        try:
            ...
            personal_details = session.execute("select id,name from personal_details")
    ...
    def faculty_signin(self,email,password):
        try:
            ...
            data = self.session.execute(query2)
    ...
    def changepassword(self,password,id):
        try:
            cluster = Cluster()
            self.session = cluster.connect("university")
            query = "update student_info set password={} where id={}".format(password,id)
            result = self.session.execute(query)
            return result
        except Exception as e:
            return e
    ...
    def faculty_profile_data(self,id):
        try:
            cluster = Cluster()
            session = cluster.connect("university")
            query1 = "select * from faculty_info where id={}".format(id)
            query2 = "select * from faculty_profile where id={}".format(id)
            query3 = "select * from faculty_connection where id={}".format(id)
    ...
    def student_info(self,id):
        try:
            cluster = Cluster()
            session = cluster.connect("university")
            personal_details = session.execute("select * from personal_details where id={}".format(id))
            religion_details = session.execute("select * from religion_details where id={}".format(id))
            caste_details = session.execute("select * from caste_details where id={}".format(id))
            income_details = session.execute("select * from income_details where id={}".format(id))
            domicile_details = session.execute("select * from domicile_details where id={}".format(id))
            permanent_address_details = session.execute("SELECT * FROM permanent_address_details where id={}".format(id))
            correspondence_address_details = session.execute("SELECT * FROM correspondence_address_details where id={}".format(id))
            past_qualification_details = session.execute("SELECT * FROM past_qualification_details where id={}".format(id))
            applying_details = session.execute("SELECT * FROM applying_details where id={}".format(id))
            return personal_details, religion_details, caste_details, income_details, domicile_details,permanent_address_details,correspondence_address_details,past_qualification_details,applying_details
        except Exception as e:
            return e